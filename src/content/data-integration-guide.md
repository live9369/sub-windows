...
> **本产品是一个前端展示框架**：不提供任何共享数据，每位用户需自行部署后端并填入自己的连接参数。  
> 目标是：按步骤接入**你自己的**数据源，并在主界面看到实时内容。  
> 左侧导航分区与「设置」页面一一对应：WeChat、Telegram、币安广场、BlockBeats、X / Twitter。

### 0) 产品定位：前端框架 + 自备数据源

- **我们提供**：分屏 UI、消息高亮、过滤、卡片布局、设置持久化、Web / 桌面双端运行
- **你不依赖我们**：所有实时数据来自你自己部署或购买的服务（wechat-decrypt、Bot 推送、WSS、API Key 等）
- **多用户适用**：每位用户填写自己的地址与鉴权，数据互不共享；本仓库不包含任何生产级后端
- **首次打开**：界面为空是正常的，接入至少一个数据源后才会有内容
...
- Web 版（线上部署，如 Vercel）：**不能**用 `http://localhost:5678` 连微信——无 Vite 代理，且 HTTPS 页面禁止请求本机 HTTP。可选方案：
  - 桌面版（Electron）或本地 `npm run dev:web`
  - 用 ngrok / Cloudflare Tunnel 将本机 wechat-decrypt 暴露为 `https://xxx` 后填入设置
  - 其余需自建 API 网关（含 CORS）
