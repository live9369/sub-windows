...
- WeChat 本地链路（桌面版 + 本机 `wechat-decrypt`）
- Telegram 用户链路（MTProto 登录）
- Telegram Bot 推送链路（外部 WS/SSE 推送）
- 币安广场链路（手动 `curl`）
- BlockBeats 新闻链路（API Key）
- X / Twitter 链路（WSS 推送）

#### 开发中链路（持续迭代）

- Web 形态下本地链路网关化接入（替代 Electron 本地桥接能力）
...
- 桌面版（推荐）：支持本地桥接（WeChat / Telegram / WSS），可直接连接你手动启动的本机后端。
- Web 版：主要用于界面展示；不包含 Electron 本地桥接能力，WeChat 等本地链路需先通过你自己的后端网关暴露为可访问接口。
