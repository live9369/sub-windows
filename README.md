...
- WeChat 本地链路：桌面版连接你手动启动的 `wechat-decrypt`（`localhost`）。
- Telegram 用户链路：MTProto 登录（`API ID` / `API Hash` / 手机号 / 验证码 / 2FA）。
- Telegram Bot 推送链路：通过外部 WS/SSE 推送服务接入（非轮询）。
- 币安广场链路：粘贴可用 `curl` 请求后拉取帖子流。
- X / Twitter 链路：接入 WSS 推送服务（URL + Token）。
- BlockBeats 链路：填写 API Key 后拉取新闻流。

#### 开发中链路（已规划并逐步迭代）

- Web 部署形态下的本地链路适配（通过网关转发本地敏感链路，而非直接桥接）。
...
- Web 构建可用于页面展示，但不具备 Electron 本地桥接能力；微信等本地链路需自建网关后再接入。
